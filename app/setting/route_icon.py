...
def function():
    ...
    icon_navbar = request.files.get('icon_navbar') # obtiene el archivo cargado para el campo de formulario "icon_navbar"
    ...
    file(icon_png, ruta) # guarda el archivo en la carpeta "icon" con el nombre "code.png"
    
    if not icon_navbar: # si hay un archivo cargado para el campo "portada"
        flash('se necesita cargar un archivo para subirlo a la plataforma') #mandamos un mensaje de que el formulario esta vacio y salimos sin guardar cambios
        return
    ruta = os.path.join(os.getcwd(), 'app', 'static', 'images', 'icon_navbar') # establece la carpeta de destino para el archivo cargado
    os.makedirs(ruta, exist_ok=True) # crea la carpeta si no existe
    file(icon_navbar, ruta) # guarda el archivo en la carpeta "icon" con el nombre "code.png"
